...
function Buttons() {
  ...
  const handleEqual = ()=>{
    ...
    let res=eval(`${calculator.display}${calculator.next}`);
    // console.log('res',res)
    setCalculator({...calculator,display:String(res),waitingForOperand:false,operation:'',next:null})
    setCalculator({display: parseInt(calculator.display.replace(/^[0-9]/g, ))})
  }
  const handleReset= ()=> {
